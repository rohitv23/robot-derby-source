...
def callback(message):
        ...
        if 'cloudTimestampMs' in dict_data and 'actions' in dict_data and 'mode' in dict_data and 'sensorRate' in dict_data:
            ...
            if dict_data['cloudTimestampMs'] > previous_command_timestamp:
                ...
                # if 'carTimestampMs' in dict_data:
                #     carTimestamp = dict_data['carTimestampMs']
                # else:
                #     carTimestamp = 0
                #
                #carTimestamp = dict_data['carTimestampMS']
...
#     image_obj = Image.open(image_path)
#     new_image = image_obj.rotate(degree)
...
def takephoto(project_id,bucket_id,cam_pos):
    ...
    #print("Current Camera ISO: " + str(camera.iso))
    ...
    print("takephoto(): Image: " + date_string)
    camera.capture(date_string)
    camera.close()  # We need to close off the resources or we'll get an error.

    local_image = date_string

    ### rotate image using Pillow lib - faster processing by using direct pi camera func
    # if cam_pos != "1":
    #    rotate(date_string,180)
    # box = (300, 600, 1400, 1200)
    # crop(local_image, box,date_string)

    client = storage.Client(project=project_id)
    mybucket = client.bucket(bucket_id)
    myblob = mybucket.blob(date_string)
    print("takephoto(): Uploading")
    # See docs: http://google-cloud-python.readthedocs.io/en/latest/storage/blobs.html
    myblob.upload_from_filename(date_string, content_type='image/jpeg')
...
# def CaptureAndUpload(project_id,bucket_id):
#     voltage = Easy_GPG.get_voltage_battery()
#     print("CaptureAndUpload(): Current Voltage: " + str(voltage))
#     distance = my_distance_sensor.read_mm()
#     print("CaptureAndUpload(): Distance Sensor Reading (mm): " + str(distance))
#     # public_image_url = takephoto(project_id, bucket_id)
#     print("CaptureAndUpload(): Uploaded image. URL: " + str(public_image_url) + " GCS URL: " + str(gcs_image_url))
#     print("\n")
...
if __name__ == '__main__':
    ...
    # command_topic = verifyEnv("COMMAND_TOPIC")
    ...
    # Initialize Robot Derby Car System and Sensors
    print("Initializing Robot Derby Car...")
    myCar = RobotDerbyCar()
    ...
    try:
        ...
        while True:
                # End loop on <ESC> key
                print("--------- main(carId=" + carId + "): balls collected='"+ str(balls_collected) +"' command: ball_color='" + ball_color + "' mode='" + mode + "' sensorRate='" + sensor_rate + "'")
                ...
                if (len(action_queue) > 0):
                    ...
                    # print("main(): ---> Start processing action= '" + str(action) + "'")
                    ...
                    if(command_timestamp>=startup_time):
                        ...
                        # print("main(): previous_command_timestamp: " + str(action[0]))
                        # print("main(): execute action: " + str(action_type) + ", Value: " + str(action_value))
                        if (action_type == "driveForwardMm"):
                            ...
                            if (myCar.drive(int(action_value),dist_limit)):
                                ...
                                obstacle_found = True
                            # time.sleep(0.2)     # Short delay to prevent overlapping commands and car confusion
                        elif (action_type == "driveBackwardMm"):
                            print("main(): Drive backward " + str(action_value) + " mm")
                            #myCar.drive_cm(int(int(action_value)/10))
                            ...
                            send_next_message = True
                            # print("main(): Will send sensor msg")
                        else:
                ...
                ###########################################################################
                ### TBD: Once commands are processed collect picture, distance, voltage ###
                ###########################################################################
                    ...
                    # print("main(): Voltage: " + str(voltage))
                    ...
                    # print("main(): Image GCS: " + str(gcs_image_url))
                    ...
                    send_next_message = False
                    print("main(): >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> Msg published to MQTT")
                    print("")
                    myCar.SetLEDsGreen()
                else:
                    # print("main(): Sleeping for " + str(sleep_time) + " seconds")
                    time.sleep(2)
