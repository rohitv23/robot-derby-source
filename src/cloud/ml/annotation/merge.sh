...
download_user_images()
{
    ...
    gsutil cp gs://$GCS_BUCKET/* ./ | true # ignore if there are no images

    if [ -z "$(ls -A $USER_FOLDER)" ]; then
        echo_my "No zip files were found or downloaded from the user GCS bucket '$GCS_BUCKET'" $ECHO_WARNING
        return
    fi
...
rename_files_in_folder()
{
    ...
    do
        # echo_my "Found pattern matching file='$file'";
        ...
        if ( stat -t $file > /dev/null 2>&1 ); then
            ...
            if [[ $NUM =~ $re ]] ; then
                ...
                if (( $MAX_NUM < $NUM )); then
                    # echo_my "Found higher index number '$NUM'"
                    MAX_NUM=$NUM
                fi
            fi
        fi
...
convert_xml_pixels()
{
    ...
    # echo_my "DEBUG XMIN=$XMIN XMAX=$XMAX YMIN=$YMIN YMAX=$YMAX"
...
resize_files()
{
    ...
    do
        ...
        if ( test "$HEIGHT" -gt "$MAX_PIXELS" ) || ( test "$WIDTH" -gt "$MAX_PIXELS" ); then
...
merge_all_images()
{
    ...
    do
        ...
        if [ -f "$DEST_FILE" ] ; then
            # echo_my "Found matching file '$FILE'"
            ...
            # echo_my "Src size='$SRC_SIZE' dest size='$DEST_SIZE'"
...
# TODO - check for imagemagic install and avoid repeated install
# We need image magic to convert file resolutions for jpg
yes | sudo apt-get install imagemagick

download_stock_images

# gsutil cp gs://robot-derby-backup/provided-images.zip.2018-05-26.22:26:47 gs://robot-derby-io-images/provided-images.zip
# rm BlueBall/* BlueHome/* GreenHome/* GreenBall/* RedBall/* RedHome/* YellowBall/* YellowHome/*
