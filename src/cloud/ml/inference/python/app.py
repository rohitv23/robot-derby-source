...
# Based on: https://cloud.google.com/solutions/creating-object-detection-application-tensorflow
...
# TODO           TODO                TODO
# Need to have different proximity thresholds for objects of the same type and of different type
...
def build_json_response(boxes, scores, classes, num_detections, image):
  ...
  for i in range(num_detections):
    if scores[i] > PROBABILITY_TRESHOLD:
      ...
      # TODO - using relative coordinates 0 to 1 - instead of the absolute ones 0 to 1024 (or whatever)
      ...
      #print "label: ",label
      #print "Didn't meet min probability threshold" 
    ...
    if k not in responses_to_remove:
      ...
      for key,val in response_msg.iteritems():
        if key != k and key not in responses_to_remove:
          ...
          #print "w_threshold:",w_threshold,", h_threshold:",h_threshold
