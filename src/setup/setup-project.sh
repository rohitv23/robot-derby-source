...
##################################################################################
generate_project_id() {
    echo "robot-derby-$(echo `date +%s | sha256sum | base64 | head -c 8`)" | tr '[:upper:]' '[:lower:]'
}
...
create_service_account() {
    ...
    gcloud iam service-accounts create $SERVICE_ACCOUNT --display-name "Robot Derby dev service account"
    ...
    # This is commented out because when run by regular user, this user will not have permission to change permissions for the Source Repo outside of his own project
    # See docs: https://cloud.google.com/source-repositories/docs/configure-access-control
    # echo "Grant Source Repo reader role..." && sleep $DELAY
    # gcloud projects add-iam-policy-binding $REPO_PROJECT_ID --member="serviceAccount:$ALLMIGHTY_SERVICE_ACCOUNT" --role="roles/source.reader"
        
    # echo "Grant GCS reader role to the originall set of Robot Derby ball images..." && sleep $DELAY
    # gcloud projects add-iam-policy-binding $REPO_PROJECT_ID --member="serviceAccount:$ALLMIGHTY_SERVICE_ACCOUNT" --role="roles/storage.objectViewer"
}

#############################################
# Create Special Robot Derby Role
#############################################
create_role() {
...
storage.objects.update"
    ...
    gcloud beta iam roles create $ROLE \
        --project $PROJECT \
        --title "Robot Derby Developer Role" \
        --description "Access to resources needed to develop and deploy Robot Derby" \
        --stage "GA" \
...
install_gcp_sdk() {
    ...
    if  [[ $(uname -a) = *" dex "* ]]; then
        echo "Detected GoPiGo environment - making changes to the GCloud install..."
        # sudo apt-get remove google-cloud-sdk | true # ignore if there is nothing to remove
...
create_appengine_app() {
    ...
    echo "Initializing AppEngines on the project..."
...
if [ -f "$INSTALL_FLAG" ]; then
    ...
    # TODO - in some cases this seems to generate error 
...
if ! [ -f $HOME/setenv-dev-workspace.sh ] ; then
    cp ./setenv-dev-workspace.sh $HOME/setenv-dev-workspace.sh
fi
...
if [ $(ask_create_project) = true ] ; then 
    # When running in a cloud shell - we are already logged in as the user, therefore no need to do it again
    # echo "Initialize GCP identity for human user who is the admin for the GCP project..."
    # gcloud init --project=${PROJECT}
...
# No need to activate the servicew account in Cloud Shell
...
# When using Cloud Shell we do not need to clone the repo - it will be cloned by hand in advance
# echo "Cloning the repo '$GIT_REPO_NAME' from project '$REPO_PROJECT_ID' into directory '$PROJECT_PATH'..."
# gcloud source repos clone $GIT_REPO_NAME $PROJECT_PATH --project=$REPO_PROJECT_ID
